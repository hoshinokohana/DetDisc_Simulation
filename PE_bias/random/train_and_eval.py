...
# Add the noise function import here
# This assumes you've adapted addRandomNoiseToTrainingSet to work with PyTorch tensors or your dataset.
from addnoise import add_random_noise
# Define the noise level, for example, 0.1 for 10% noise
noise_level = 0.5
...
def train(args, encoder, class_out, conf_out, device, train_loader, optimizer, epoch):
	...
	for batch_idx, (data, target) in enumerate(train_loader):
		...
		x = (x - 0.5) / 0.5
		# Add noise
		x = add_random_noise(x, noise_level)
		# Threshold image
		x = nn.Hardtanh()(x)
...
def main():
	...
	parser.add_argument('--signal_range', type=list, default=[0.0, 1.0])
	parser.add_argument('--signal_range_test', type=list, default=[0.0, 1.0])
	parser.add_argument('--signal_N_test', type=int, default=100)
	parser.add_argument('--noise_level', type=float, default=0.4, help='Noise level for training labels')
	...
	parser.add_argument('--noise_range', nargs=2, type=float, default=[0.0, 0.4], help='Noise range as two floats: min max')
	parser.add_argument('--noise_N_test', type=int, default=1, help='Number of noise levels to test')
	...
	test_acc_noiseless, _, __, ___ = test(args, encoder, class_out, conf_out, device, test_loader, signal=0, noise=0)
